...
std::array<double, 3>  TTMagneticField::at_position(const double &x, const double &y, const double &z) const { 
    ...
    double f_z=sign*std::exp(-(std::abs(z)/b_z0));
    ...
    double b_r=b_b0*(b_Rsun/(r*std::cos(phase)));

    //if(r<b5_r_min) {b_r=b5_b0*(b5_Rsun/(b5_r_min));} <-- hammurabi comment
    if (r < b_r_min) {
        b_r = b_b0 * (b_Rsun / (b_r_min * std::cos(phase)));
        }
  
    double B_r_phi = b_r*std::cos(phi-beta*log(r/b_Rsun)+phase);
