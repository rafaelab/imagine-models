...
vector TTMagneticField::_at_position(const double &x, const double &y, const double &z, const TTParams &p) const { 
    ...
    number beta = 1./tan(p.b_p);
    if(abs(p.b_p) < 1.e-16) { 
        beta = 1.;
        }

    number phase = (beta * log(1. + p.b_d / p.b_Rsun)) - M_PI/2.;
    ...
    number f_z = sign * exp(-(abs(z) / p.b_z0));
    ...
    number b_r = p.b_b0 * (p.b_Rsun/(r * cos(phase)));
    ...
    number B_r_phi = b_r * cos(phi - beta * log(r / p.b_Rsun) + phase);
